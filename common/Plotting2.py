...
applyFracColors=False, legendWidthMultiplier=1):
    ...
    if histos[0][0] in signal:
        cumul = copy.deepcopy(get_max_y_histo(histos))
        cumul.SetFillColorAlpha(33, 0.6)
        applySignalHistStyle(histos[0][1], histos[0][0], width=3, applyFracColors=applyFracColors)
        histos[0][1].SetName(histos[0][0])
        stack = R.THStack("stack", "")
        stack.Add(copy.deepcopy(cumul))
    else:
        cumul = copy.deepcopy(histos[0][1])
        cumul.SetFillColorAlpha(33, 0.6)
        applyHistStyle(histos[0][1], histos[0][0], applyFracColors=applyFracColors)
        histos[0][1].SetName(histos[0][0])
        stack = R.THStack("stack", "")
    ...
    for h in histos[1:]:
        if h[0] in signal:
            "Calling applySignalHistStyle:"
            applySignalHistStyle(h[1], h[0], width=3, applyFracColors=applyFracColors)
            h[1].SetName(h[0])
        else:
            "Calling applyHistStyle:"
            applyHistStyle(h[1], h[0], applyFracColors=applyFracColors)
            h[1].SetName(h[0])
    ...
    legWidth = 100.0*legendWidthMultiplier
    relLegWidth = legWidth / width

    legMargin = 0.6 / legendWidthMultiplier
        
    # if constrainLegend:
    #     leg = R.TLegend(1 - relLegWidth * 1.5 - 0.05, 0.65, 1 + relLegWidth - 0.05, 1 - topMargin - 0.01)  
    # else:
    #     leg = R.TLegend(1 - relLegWidth * 1.5 - 0.05, 0.4, 1 + relLegWidth - 0.05, 1 - topMargin - 0.01) 

    if constrainLegend:
        leg = R.TLegend(1 - relLegWidth * (1 + legMargin) - 0.1, 0.65, 1 - 0.1, 1 - topMargin - 0.01)  
    else:
        leg = R.TLegend(1 - relLegWidth * (1 + legMargin) - 0.1, 0.4, 1 - 0.1, 1 - topMargin - 0.01) 

    # if constrainLegend:
    #     leg = R.TLegend(1 - relLegWidth * 1.0 - 0.05, 0.65, 1 + relLegWidth - 0.05, 1 - topMargin - 0.01)  
    # else:
    #     leg = R.TLegend(1 - relLegWidth * 1.0 - 0.05, 0.4, 1 + relLegWidth - 0.05, 1 - topMargin - 0.01) 

    # if constrainLegend:
    #     leg = R.TLegend(1 - relLegWidth * 2.0 - 0.05, 0.7, 1 + relLegWidth - 0.05, 1 - topMargin - 0.01)  
    # else:
    #     leg = R.TLegend(1 - relLegWidth * 2.0 - 0.05, 0.7, 1 + relLegWidth - 0.05, 1 - topMargin - 0.01) 
    ...
    if legend == "outer":
        ...
        if constrainLegend:
            ...
            leg = R.TLegend(1 - rightMargin, 0.4, 1 - rightMargin + relLegWidth * 2.5, 1 - topMargin) 

        leg.SetMargin(0.2 / legendWidthMultiplier)
...
def get_max_y_histo(histos):
    max = 0
    maxHisto = None
    for h in histos:
        print "histo max is {0}".format(h[1].GetMaximum())
        if h[1].GetMaximum() > max:
            max = h[1].GetMaximum()
...
def applyHistStyle(hist, name, applyFracColors=False):
    ...
    if applyFracColors:
        hist.SetFillColor( getFractionAssociatedColor( name ) )
    else:
...
def applySignalHistStyle(hist, name, width = 1, applyFracColors=False):
    ...
    hist.GetYaxis().SetLabelSize(14)
    if applyFracColors:
        hist.SetLineColor( getFractionAssociatedColor( name ) )
    else:
...
def getFancyName(name):
    ...
    if name in "QCD_estimate":  return "QCD (jet #rightarrow #tau_{h})"
    if name in "W_anti":              return "W (jet #rightarrow #tau_{h})"
    if name in "TTJ_anti":              return "t#bar{t} (jet #rightarrow #tau_{h})"
    if name in "VVJ_anti":              return "VV (jet #rightarrow #tau_{h})"
    if name in "ZJ_anti":              return "Z (jet #rightarrow #tau_{h})"
    if name in "ZTT_anti":              return "Z (#tau_{h} #rightarrow #tau_{h})"
    if name in "VVT_anti":              return "VV (#tau_{h} #rightarrow #tau_{h})"
    if name in "TTT_anti":              return "t#bar{t} (#tau_{h} #rightarrow #tau_{h})"
    if name in "ZL_anti":              return "Z (l #rightarrow #tau_{h})"
    if name in "VVL_anti":              return "VV (l #rightarrow #tau_{h})"
    if name in "TTL_anti":              return "t#bar{t} (l #rightarrow #tau_{h})"
    if name in "EMB_anti":              return "#tau-Embedding"
    ...
    if name in "origin_real":           return "Z, VV, t#bar{t} (l #rightarrow #tau_{h}, #tau_{h} #rightarrow #tau_{h})"
    if name in "origin_w":           return "W, Z, VV (jet #rightarrow #tau_{h})"
    if name in "origin_qcd":           return "QCD (jet #rightarrow #tau_{h})"
    if name in "origin_tt":           return "t#bar{t} (jet #rightarrow #tau_{h})"    

    return name


def getFractionAssociatedColor(name):
    # print "Name in getColor is:"
    # print name
    if name in ["TTJ_anti", "TTJ", "origin_tt"]:    return R.TColor.GetColor(155,152,204)
    if name in ["W_anti", "W", "ZJ_anti", "ZJ", "VVJ_anti", "VVJ", "origin_w"]:                  return R.TColor.GetColor(222,90,106)
    if name in ["QCD_estimate", "QCD", "origin_qcd"]:        return R.TColor.GetColor(250,202,255)
    if name in ["ZTT_anti", "ZT", "ZL_anti", "ZL", "EMB_anti", "EMB", "TTT_anti", "TTT", "TTL_anti", "TTL", "VVT_anti", "VVT", "VVL_anti", "VVL", "origin_real"]:                 return R.TColor.GetColor(248,206,104)
